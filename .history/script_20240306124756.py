...
def combine_excel_files(path):
    ...
    with pd.ExcelWriter(output_filename, engine='xlsxwriter') as writer:
      ...
      # Add original columns from overall_results.xlsx to the scenario_name_comparison sheet
        overall_columns = df.filter(like='_overall_results.xlsx$', axis=1)
        overall_columns.loc[:, 'Scenario'] = scenario_name  # Use .loc to avoid the warning
        ...
        calculations_table.to_excel(writer, sheet_name=f"{scenario_name}_comparison", startrow=len(revenues)+len(overall_columns)+5, index=False)
